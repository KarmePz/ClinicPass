...
export const Reportes: React.FC = () => {
  ...
  const fetchStats = async (filtroFecha: FiltroFechaDTO) => {
    try {
      // setLoading(true);

      const totalTurnos = await getTurnosPeriodo(filtroFecha);
      const pacientesAtendidos = await getPacientesAtendidos(filtroFecha);
      const fichasCompletadas = await getNumeroFichasFiltro(filtroFecha);
  ...
  const handlePeriodoChange = async (
    ...
    const nuevoPeriodo = e.target.value as unknown as FiltroFecha;
  ...
  const handleAplicarFechasPersonalizadas = async () => {
    ...
    await fetchStats({
      ...
      // "Envolvemos" el string para convertirlo en objeto Date
